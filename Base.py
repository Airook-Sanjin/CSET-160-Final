...
def createAccount():   
    ...
    try:
        RadioValue= request.form["Teach-Stud"]
        if RadioValue == "1":
            prevID = conn.execute(text("select Sid from student order by Sid desc Limit 1;")).one()
            ...
            prevID = conn.execute(text("select Tid from teacher order by Tid desc Limit 1;"))
            newID = int(prevID[0])+1
            conn.execute(text("insert into teacher values(:Tid, :first_name, :last_name, :password)"), {"Tid": newID, "first_name":request.form["first_name"], "last_name":request.form["last_name"],"password":request.form["password"]})
            # print(TeacherTbl)
